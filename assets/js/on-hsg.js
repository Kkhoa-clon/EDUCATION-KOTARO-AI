...
class EbookManager {
    ...
    async loadBooks() {
        try {
            const response = await fetch('../../assets/json/on-hsg.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            this.books = await response.json();
            this.filteredBooks = [...this.books];
            console.log(`Loaded ${this.books.length} books from on-hsg.json`);
        } catch (error) {
            console.error('Error loading books:', error);
            this.showError('Không thể tải dữ liệu sách từ on-hsg.json. Vui lòng thử lại sau.');
    ...
    exportBooksToFile() {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = 'on-hsg.json';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        
        this.showSuccess('Đã xuất dữ liệu sách thành công! Vui lòng thay thế file on-hsg.json hiện tại.');
    }

    // Method to reload books from on-hsg.json
    async reloadBooksFromFile() {
        try {
            await this.loadBooks();
            this.renderBooks();
            this.updateStats();
            this.showSuccess('Đã tải lại dữ liệu từ on-hsg.json thành công!');
        } catch (error) {
            this.showError('Không thể tải lại dữ liệu từ on-hsg.json.');
        }
    }
}

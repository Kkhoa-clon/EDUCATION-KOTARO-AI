...
class EbookManager {
    ...
    async loadBooks() {
        try {
            const response = await fetch('../../assets/json/sinh-hoc.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            this.books = await response.json();
            this.filteredBooks = [...this.books];
            console.log(`Loaded ${this.books.length} books from sinh-hoc.json`);
        } catch (error) {
            console.error('Error loading books:', error);
            this.showError('Không thể tải dữ liệu sách từ sinh-hoc.json. Vui lòng thử lại sau.');
    ...
    exportBooksToFile() {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = 'sinh-hoc.json';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        
        this.showSuccess('Đã xuất dữ liệu sách thành công! Vui lòng thay thế file sinh-hoc.json hiện tại.');
    }

    // Method to reload books from sinh-hoc.json
    async reloadBooksFromFile() {
        try {
            await this.loadBooks();
            this.renderBooks();
            this.updateStats();
            this.showSuccess('Đã tải lại dữ liệu từ sinh-hoc.json thành công!');
        } catch (error) {
            this.showError('Không thể tải lại dữ liệu từ sinh-hoc.json.');
        }
    }
}

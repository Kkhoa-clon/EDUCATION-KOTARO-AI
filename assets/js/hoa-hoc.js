...
class EbookManager {
    ...
    async loadBooks() {
        try {
            const response = await fetch('../../assets/json/hoa-hoc.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            this.books = await response.json();
            this.filteredBooks = [...this.books];
            console.log(`Loaded ${this.books.length} books from hoa-hoc.json`);
        } catch (error) {
            console.error('Error loading books:', error);
            this.showError('Không thể tải dữ liệu sách từ hoa-hoc.json. Vui lòng thử lại sau.');
    ...
    exportBooksToFile() {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = 'hoa-hoc.json';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        
        this.showSuccess('Đã xuất dữ liệu sách thành công! Vui lòng thay thế file hoa-hoc.json hiện tại.');
    }

    // Method to reload books from hoa-hoc.json
    async reloadBooksFromFile() {
        try {
            await this.loadBooks();
            this.renderBooks();
            this.updateStats();
            this.showSuccess('Đã tải lại dữ liệu từ hoa-hoc.json thành công!');
        } catch (error) {
            this.showError('Không thể tải lại dữ liệu từ hoa-hoc.json.');
        }
    }
}

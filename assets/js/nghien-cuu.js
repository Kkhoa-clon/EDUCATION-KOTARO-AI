...
class EbookManager {
    ...
    async loadBooks() {
        try {
            const response = await fetch('../../assets/json/nghien-cuu.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            this.books = await response.json();
            this.filteredBooks = [...this.books];
            console.log(`Loaded ${this.books.length} books from nghien-cuu.json`);
        } catch (error) {
            console.error('Error loading books:', error);
            this.showError('Không thể tải dữ liệu sách từ nghien-cuu.json. Vui lòng thử lại sau.');
    ...
    exportBooksToFile() {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = 'nghien-cuu.json';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        
        this.showSuccess('Đã xuất dữ liệu sách thành công! Vui lòng thay thế file nghien-cuu.json hiện tại.');
    }

    // Method to reload books from nghien-cuu.json
    async reloadBooksFromFile() {
        try {
            await this.loadBooks();
            this.renderBooks();
            this.updateStats();
            this.showSuccess('Đã tải lại dữ liệu từ nghien-cuu.json thành công!');
        } catch (error) {
            this.showError('Không thể tải lại dữ liệu từ nghien-cuu.json.');
        }
    }
}

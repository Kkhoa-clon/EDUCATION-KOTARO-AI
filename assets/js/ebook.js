...
class EbookManager {
    ...
    async loadBooks() {
        try {
            const response = await fetch('../../assets/json/ebooks.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            this.books = await response.json();
            this.filteredBooks = [...this.books];
            console.log(`Loaded ${this.books.length} books from ebooks.json`);
        } catch (error) {
            console.error('Error loading books:', error);
            this.showError('Không thể tải dữ liệu sách từ ebooks.json. Vui lòng thử lại sau.');
    ...
    exportBooksToFile() {
        ...
        this.showSuccess('Đã xuất dữ liệu sách thành công! Vui lòng thay thế file ebooks.json hiện tại.');
    }

    // Method to reload books from ebooks.json
    async reloadBooksFromFile() {
        try {
            await this.loadBooks();
            this.renderBooks();
            this.updateStats();
            this.showSuccess('Đã tải lại dữ liệu từ ebooks.json thành công!');
        } catch (error) {
            this.showError('Không thể tải lại dữ liệu từ ebooks.json.');
        }
    }
}
